...
class Sensor2(object):
    ...
    def poll(self):
        ...
        if now > self.t_next:
            ...
            # For DHT sensors, assign the value to the DHT message field
            if self.message_type == MessageType.DHT:
                self.msg.Humidity = self.value[0]
                self.msg.Temperature = self.value[1]
            else:
                self.msg.value = self.value

            # Add a timestamp and publish the message
            print(self.msg.Humidity," ",self.msg.Temperature)
            self.msg.header.stamp = rospy.Time.now()
            self.pub.publish(self.msg)
...
class DHT22(Sensor2):
    def __init__(self, *args, **kwargs):
        ...
        self.message_type = MessageType.DHT

        self.msg = dht_22()
        self.msg.header.frame_id = self.frame_id

        self.pub = rospy.Publisher("~sensor/" + self.name, dht_22, queue_size=5)
    ...
    def read_value(self):
        return self.controller.dht_read(self.pin)
